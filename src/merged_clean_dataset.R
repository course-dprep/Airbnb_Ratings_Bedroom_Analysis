...
listings_selected %>% group_by(host_response_time) %>% summarise(n())
...
listings_selected %>% group_by(host_response_time) %>% summarise(mean(review_scores_communication)) # Not really suitable

ggplot(data = listings_selected, aes(x = avg_price, y = review_scores_cleanliness)) +
...
ggplot(data = listings_selected, aes(x = avg_price, y = review_scores_location)) +
  geom_point() + # Add points for the scatter plot
  geom_smooth(method = "lm", se = FALSE) + # Add a linear regression line (change method as needed)
